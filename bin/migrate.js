...
入力は $T = 50$ 行与えられます。
$i$ 行目には、\`x\` と \`+\` で構成される $N = 20$ 文字の多項式風文字列 $S_i$ が与えられます。
...
$i$ 行目には、 $S_i$ が多項式 $f_i$ とみなされるとしたとき $f_i'$ とみなされる多項式風文字列を出力してください。
...
入力例の多項式風文字列はそれぞれ $x^3, x^4+x^2, 2x^2$ とみなされます。これを $x$ で微分したものはそれぞれ $3x^2, 4x^3+2x, 4x$ です。

実際、出力例の多項式風文字列はそれぞれ $3x^2, 4x^3+2x, 4x$ とみなされ、条件を満たします。出力は「整理」されていなくても良いこと、出力における \`x\` と \`+\` および改行文字 (文字コード 0x0a) 以外の文字は採点の際無視されることに気をつけてください。

なお、この入力例は入出力の説明のためのものであり、文字列の長さと行数の観点についてテストケースの制約を**満たしていない**ことに注意してください。従って、ジャッジに使われるテストケースの入力はこの入力例の通りには**なりません**。
...
この入力例は制約を**満たしています**。従って、ジャッジに使われるテストケースの入力はこの入力例の通りに**なる可能性があります**。
				`,
				en: '',
			},
		},
